//
//  DSPF_TourTableViewCell.m
//  Hermes
//
//  Created by iLutz on 06.08.11.
...
@implementation DSPF_TourTableViewCell_CCC
...
- (void)setTourDeparture:(Departure *)newDeparture {
    ...
    if ([self.tourDeparture.canceled boolValue]) { 
        ...
        if (self.tourDeparture.departure) {
            ...
            isPickUpOnly = ([[self.tourDeparture.location_id.transport_group_addressee_id allObjects]
                             filteredArrayUsingPredicate:
                             [NSPredicate predicateWithFormat:@"pickUpAction = nil && deliveryAction != nil"]].count == 0);
            if (isPickUpOnly && [self.tourTask isEqualToString:TourTaskLoadingOnly]) {
                departureLabel.text = @"❎";
    ...
    if ([self.tourDeparture.currentTourStatus intValue] < 50) {
        if ((self.pickCountForTourLocation + self.unitCountForTourLocation + self.palletCountForTourLocation) != 0) {
            ...
            self.infoLabel.text = [NSString stringWithFormat:@"⬆ %i  ⬇ %i %@", 
    ...
    if (([self.tourDeparture.currentTourStatus intValue] == 20 && [self.tourTask isEqualToString:TourTaskLoadingOnly]) ||
    ...
    } else  if ((isPickUpOnly && [self.tourTask isEqualToString:TourTaskLoadingOnly]) ||
                ([self.tourDeparture.currentTourStatus intValue] == 30 && [self.tourTask isEqualToString:TourTaskLoadingOnly]) ||
